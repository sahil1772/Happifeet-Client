// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'ClientTheme.dart';

// **************************************************************************
// JsonSerializableGenerator
// **************************************************************************

ClientTheme _$ClientThemeFromJson(Map<String, dynamic> json) => ClientTheme(
      logo: json['logo'] as String?,
      background_color: json['background_color'] as String?,
      button_background: json['button_background'] as String?,
      button_text_color: json['button_text_color'] as String?,
      search_filter_border: json['search_filter_border'] as String?,
      top_title_background_color: json['top_title_background_color'] as String?,
      top_title_text_color: json['top_title_text_color'] as String?,
      title_color_on_listing: json['title_color_on_listing'] as String?,
      body_text_color: json['body_text_color'] as String?,
      menu_active_color: json['menu_active_color'] as String?,
      thank_you_image_color: json['thank_you_image_color'] as String?,
      text_color: json['text_color'] as String?,
      thank_you_page_message: json['thank_you_page_message'] as String?,
    );

Map<String, dynamic> _$ClientThemeToJson(ClientTheme instance) =>
    <String, dynamic>{
      'logo': instance.logo,
      'background_color': instance.background_color,
      'button_background': instance.button_background,
      'button_text_color': instance.button_text_color,
      'search_filter_border': instance.search_filter_border,
      'top_title_background_color': instance.top_title_background_color,
      'top_title_text_color': instance.top_title_text_color,
      'title_color_on_listing': instance.title_color_on_listing,
      'body_text_color': instance.body_text_color,
      'menu_active_color': instance.menu_active_color,
      'thank_you_image_color': instance.thank_you_image_color,
      'text_color': instance.text_color,
      'thank_you_page_message': instance.thank_you_page_message,
    };
