...
class AddClientWidget extends StatefulWidget{
  ...
  gotoAddClientPage(BuildContext context,String? id, bool? isEdit){
    Navigator.push(context, MaterialPageRoute(builder: (_) => AddClientWidget(id: id,isEdit: isEdit,)));
  }
...
class _AddClientWidgetState extends State<AddClientWidget>{
  ...
  Future<void> getClientDataForEdit() async{
    var response = await ApiFactory().getClientService().editClientUserData("edit_client_users", widget.id!);
...
setInitialData(){
    ...
    if(editClientData!.email_address != null){
      setEmailError(EmailValidator.validate(editClientData!.email_address!) ? null : "Enter valid email id");
                                  ...
                                  child: FlutterSwitch(
                                      ...
                                      onToggle: (value) {
                                        setState(() {
                                          log("toggle ${value}");
                                          if(value){
                                            addClientUser.email_notification = "Y";
                                          }else{
                                            addClientUser.email_notification = "N";
                                          }
                                  ...
                                  child: ElevatedButton(
                                    ...
                                    onPressed: () async{
                                      if(widget.isEdit!){
                                        ...
                                        dataToUpdate.client_name = clinetNameController.text;
                                        dataToUpdate.email_address = emailController.text;
                                        dataToUpdate.contact_no = contactNoController.text;
                                        dataToUpdate.username = usernameController.text;
                                        dataToUpdate.password = confirmpasswordController.text;
                                    ...
                                    if(_formkey.currentState!.validate()){
                                      var response = await ApiFactory().getClientService().updateClientUserData(dataToUpdate);
                                      if(response.status == "1"){
                                        log("CLIENT DATA UPDATED SUCCESSFULLY!");
                                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("CLIENT data updated successfully")));
                                        Future.delayed(
                                            Duration(seconds: 2), () {
                                              // Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => ClientListingWidget()));
                                      ...
                                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Please enter required field")));
                                        ...
                                        if(_formkey.currentState!.validate()){
                                          ...
                                          var response = await ApiFactory().getClientService().submitClientUserData(addClientUser);
                                          if(response.status == "1"){
